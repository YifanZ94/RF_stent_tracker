...
for i = 1:size(position,2)
    subplot(j,k,i); surface(X,Y,data(:,:,i)); title(sprintf('Power at Position = %d in', position(i))); xlabel('Frequency (GHz)'); ylabel('Trial Number'); h=colorbar; ylabel(h,'Power (dBm)');
end
...
for i = 1:size(position,2)
    ...
    plot(frequencyscale,averages(1,:,i)-2*standard_devs(1,:,i), '--','Color', [0.8500,0.3250,0.0980]); %plots -2 std deviations
    title(sprintf('Averaged Power at Position = %d in (%d Trials)', position(i), size(data,1)));
...
for i = 1:size(position,2)
    ...
    xlabel('Frequency (GHz)'); ylabel('Power (dBm)'); 
    Legend{i}=sprintf('x = %d in',position(i));
    hold off
end
